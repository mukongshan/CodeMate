...
export function SourceControlPanel() {
  const { sessionId, currentLane, lanes, addToast } = useStore();
...
export function SearchPanel() {
  ...
  return <div className="flex h-full min-h-0 flex-col"><div className="border-b border-border px-3 py-2"><div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-text-muted">搜索</div><div className="mt-0.5 text-xs text-text-secondary">在当前 Lane 工作区查找</div></div><div className="p-3"><div className="flex items-center gap-2 rounded border border-border bg-surface-1 px-2 py-1.5"><Search className="h-4 w-4 text-text-muted" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-xs outline-none" placeholder="搜索文件名或内容" aria-label="搜索文件名或内容" /></div><div className="mt-3 rounded border border-border bg-surface-1 p-3 text-xs text-text-muted"><CircleAlert className="mb-2 h-4 w-4 text-status-warning" /><p>结构化搜索接口尚未接入。</p><p className="mt-1">当前先保留入口和交互位置，后续接入结果列表及编辑器行号定位。</p>{query && <p className="mt-2 text-text-secondary">查询：{query}</p>}</div></div></div>;
}
