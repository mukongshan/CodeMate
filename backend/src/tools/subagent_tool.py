...
class DelegateTaskTool(Tool):
    name = "delegate_task"
    description = (
        "优先把需要跨文件追踪、影响范围分析、风险排查、测试失败定位或只读代码审查的"
        "调查任务委托给独立的子 Agent。子 Agent 只能使用只读工具（读文件、搜索），"
        "完成后只返回一段结论摘要，不会把中间过程带回来。一个调用只负责一个边界清楚的"
        "主题；多个互不依赖的主题应在同一轮并行委托。不要用它处理简单单文件读取、写入、"
        "编辑或命令执行。适合『帮我确认 XXX』『帮我调查 XXX 有没有问题』这类任务。"
    )
    parameters = {
        "type": "object",
        "properties": {
