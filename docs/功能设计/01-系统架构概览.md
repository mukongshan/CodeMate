...
flowchart TB
    ...
    subgraph AgentCore[Agent 核心层]
        ...
        LLM[LLMClient\n重试与错误归一化]
        Provider[OpenAIProvider / DeepSeekProvider\n流式协议适配]
        Tools[ToolRegistry\n内置工具注册与执行]
        Delegate[delegate_task\n只读子 Agent]
    end

    subgraph Safety[安全与执行支撑层]
        Permission[PermissionManager\nSAFE / WRITE / DANGEROUS]
        Rules[权限规则与命令黑名单]
        FileTools[文件工具\n路径与版本校验]
        Exec[命令执行器\n超时 / cwd / 输出控制]
    ...
    subgraph Data[数据与版本管理层]
        WorkspaceStorage[WorkspaceStorage\n注册表 / SessionPaths / 删除 Journal]
        SessionStorage[SessionStorage\nEntry 索引与上下文窗口]
        LaneManager[LaneManager\nLane 指针与流水]
        GitManager[GitLaneManager\n分支 / Worktree / 检查点 / 集成]
        WorkspaceFiles[workspace_files\n文件浏览 / 搜索 / 保存]
    end

    subgraph Persistence[本地持久化]
        Files[(JSON / JSONL / NDJSON)]
        ProjectWorkspace[(用户代码工作区)]
        Worktrees[(Git Worktree 与仓库对象)]
    end

    subgraph External[外部服务]
        LLMService[(OpenAI / DeepSeek\n兼容 LLM API)]
        SearchService[(Bocha / Volcengine\n网页搜索服务)]
    end
    ...
    Manager --> WorkspaceStorage
    SessionRuntime --> Loop
    SessionRuntime --> Memory
    SessionRuntime --> Naming
    SessionRuntime --> Permission
    SessionRuntime --> GitManager
    SessionRuntime --> Terminal
    SessionRuntime --> Logger
    SessionRuntime --> SessionStorage
    SessionRuntime --> LaneManager
    ...
    Providers --> LaneManager
    Memory --> SessionStorage
    Memory --> LaneManager
    WorkspaceFiles --> GitManager
    WorkspaceFiles --> ProjectWorkspace
    SessionStorage --> Files
    LaneManager --> Files
    WorkspaceStorage --> Files
    GitManager --> Files
    GitManager --> ProjectWorkspace
    GitManager --> Worktrees
    Exec --> ProjectWorkspace
    Logger --> Files
```
