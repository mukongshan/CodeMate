...
assistant: [text] + [tool_use: read(a.py)] + [tool_use: read(b.py)]   ← 1 个节点
  ↓
user(synthetic): [tool_result: a.py 内容] + [tool_result: b.py 内容]  ← 打包成 1 个节点，不是 2 个
  ↓
...
一次完整问答（用户提问到最终回复）通常对应树上的**一条链**，链上有好几个节点（user → assistant(带tool_use) → user(带tool_result) → assistant(最终文本)），不是一个节点。
